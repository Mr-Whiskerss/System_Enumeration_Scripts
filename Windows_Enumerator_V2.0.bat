@echo off
:: ============================================================
::  Windows Enumerator V2.0  -  Self-Contained Launcher
::  For authorised security assessments only.
::
::  Usage:
::    Windows_Enumerator_V2.0.bat
::    Windows_Enumerator_V2.0.bat -Extended
::    Windows_Enumerator_V2.0.bat -OutputFile C:\Temp\out.txt
::    Windows_Enumerator_V2.0.bat -Extended -Domain -OutputFile C:\Temp\out.txt
::    Windows_Enumerator_V2.0.bat -NoBanner
::
::  This file is self-contained. It extracts and runs the embedded
::  PowerShell payload from itself, then cleans up the temp file.
:: ============================================================
setlocal

:: --- Temp file for extracted PS payload ---
set "_PS_TMP=%temp%\WinEnum_%RANDOM%%RANDOM%.ps1"

:: --- Extract everything after the ##PSSTART## marker into temp PS1 ---
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
    "$raw = Get-Content '%~f0' -Raw -Encoding UTF8; ^
     $idx = $raw.IndexOf('##PSSTART##'); ^
     if ($idx -lt 0) { Write-Error 'Payload marker not found in bat file'; exit 1 }; ^
     $raw.Substring($idx + 11).TrimStart("`r`n") | ^
     Set-Content -Path '%_PS_TMP%' -Encoding UTF8"

if not exist "%_PS_TMP%" (
    echo [!] ERROR: Failed to extract PowerShell payload. Aborting.
    pause
    exit /b 1
)

:: --- Execute the extracted script, passing all bat arguments through ---
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%_PS_TMP%" %*

:: --- Clean up temp file ---
del /f /q "%_PS_TMP%" 2>nul
...
::  Everything after ##PSSTART## is the PowerShell payload
:: ════════════════════════════════════════════════════════════
##PSSTART##
<#
.SYNOPSIS
...
.EXAMPLE
    ...
    Windows_Enumerator_V2.0.bat -Extended -OutputFile C:\Temp\enum.txt -Domain
    Windows_Enumerator_V2.0.bat -NoBanner
...
if ($OutputFile -eq "" -and $IsInteractive) {
    ...
    if ($saveToFile -match '^(y|yes)$') {
        ...
        $OutputFile = "$env:TEMP\WindowsEnum_$timestamp.txt"
    }
...
Safe-Invoke "netstat" { netstat -ano | Select-String "ESTABLISHED" }

Write-SubHeader "Listening Ports"
Safe-Invoke "Listening" { netstat -ano | Select-String "LISTENING" }
...
Safe-Invoke "UAC" {
    ...
    $uac | Format-List
    if ($uac.EnableLUA -eq 0) { Write-Finding "UAC is DISABLED" 'HIGH' }
...
Write-SubHeader "Scheduled Tasks (Non-Microsoft, Writable Binaries)"
...
Write-SubHeader "DLL Hijacking - User-Writable Directories"
Safe-Invoke "DLL hints" {
    $dllHijackPaths = @("C:\Windows\Temp", "$env:TEMP", "$env:APPDATA")
    foreach ($path in $dllHijackPaths) {
        if (Test-Path $path) {
            try {
                $testFile = Join-Path $path "pentest_write_test_$([guid]::NewGuid().ToString('N')).tmp"
                [IO.File]::Create($testFile).Close()
                Remove-Item $testFile -Force -ErrorAction SilentlyContinue
...
Safe-Invoke "Defender" {
    ...
    if ($defPref) {
        ...
        Write-Output "  Script block scanning : $($defPref.DisableScriptScanning)"
...
Safe-Invoke "PS security settings" {
    ...
    if ($sbLog.EnableScriptBlockLogging -ne 1) { Write-Finding "ScriptBlock logging is NOT enabled" 'MEDIUM' }
...
Safe-Invoke "World-writable folders" {
    ...
    ForEach-Object {
        try {
            ...
            if ($worldWrite) { Write-Finding "Weak ACL: $($_.FullName)" 'HIGH' }
        } catch {}
    }
}
...
if ($Extended) {
    ...
    Write-SubHeader "Recently Modified Files (Last 20, C:\Users)"
